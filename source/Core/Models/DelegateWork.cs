...
namespace Bingosoft.TrioFramework.Workflow.Core {
	...
	public class DelegateWork {
		...
		public static IList<DelegateWork> GetByDeletagor(int appCode, string delegatorId) {
			return _dao.QueryEntities<DelegateWork>("trio.workflow.delegate.getby.delegatorid", new { AppCode = appCode, DelegatorId = delegatorId });
		}
		...
		public static IList<DelegateWork> GetByMandatary(int appCode, string mandataryId) {
			return _dao.QueryEntities<DelegateWork>("trio.workflow.delegate.getby.mandataryid", new { AppCode = appCode, MandataryId = mandataryId });
		}
		...
		public static bool IsDelegate(int appCode, string delegatorId, string mandataryId) {
			var rowCount = _dao.QueryScalar<int>("trio.workflow.delegate.is.delegate", new {AppCode = appCode,DelegatorId = delegatorId, MandataryId = mandataryId});
			return rowCount > 0;
		}
		...
		internal bool AddNew(){
			var effectRows = _dao.Insert(this);
			return effectRows > 0;
		}
	}
