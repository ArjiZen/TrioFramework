define('trio', ['jquery', 'template', 'bootstrap'], function ($, template) {
    ...
    var self = {
        ...
        /*控制加载数据遮罩层的显示隐藏*/
        loading: function (elementSelector, status) {
            var backdropHtml = "<div class='loading-backdrop fade in'><div class='loading'><span>处理中，请稍候...</span><br/><img src='/mas/Content/images/loading.gif'/></div></div>";
            var backdrop = $(elementSelector).find('.loading-backdrop');
            if (status === "show") {
                if (backdrop.length === 0) {
                    $(elementSelector).prepend(backdropHtml);
                }
            } else if (status === "hide") {
                if (backdrop.length > 0) {
                    backdrop.remove();
    ...
    $.fn.extend({
        ...
        /*
         * 说明：在body元素中弹出加载数据的遮罩层
         * 用法：
         * $.loading('show')
         */
        loading: function (status) {
            self.loading($(this), status);
        },
        /* 
         * 说明：使用json对象为表单赋值
         * 用法：
