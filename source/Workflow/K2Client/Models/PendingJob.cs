...
namespace Bingosoft.TrioFramework.Workflow.K2Client.Models
{
    ...
    public class PendingJob
    {
        ...
        private bool AddNew()
        {
            ...
            url = Regex.Replace(url, @"\{appCode\}", this.AppCode.ToString(), RegexOptions.IgnoreCase);
            ...
            this.Url = url;
            var effectRows = _dao.Insert<PendingJob>(this);
            return effectRows > 0;
        }
        ...
        /// <param name="todoItem">下一环节待办项</param>
        public static bool Todo(WorkflowInstance instance, WorkflowItem todoItem)
        {
            var job = new PendingJob() {
                InstanceNo = todoItem.InstanceNo,
                ...
                Result = "",
                Url = null,
                UserId = string.IsNullOrWhiteSpace(todoItem.MandataryId) ? todoItem.PartId : todoItem.MandataryId,
                UserName = string.IsNullOrWhiteSpace(todoItem.Mandatary) ? todoItem.PartName : todoItem.Mandatary
            };
            return job.AddNew();
        }
