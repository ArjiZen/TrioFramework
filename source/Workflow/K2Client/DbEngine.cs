        ...
        public override bool RunWorkflow(WorkflowInstance instance, ApproveResult result, IList<IUser> nextStepUsers, IList<IUser> tobeReadUsers)
        {
            ...
            foreach (var workItem in curWorkItems)
            {
                ...
                else
                {
                    ...
                    Logger.LogOperator("TrioFramework", Logger.BusinessAction.其他, string.Format("RunWorkflow => {0}|{1}|{2}|{3}({4}) StackTrace => {5}", 
                        workItem.InstanceNo, 
                        workItem.TaskId, 
                        workItem.CurrentActi, 
                        workItem.PartName,
                        workItem.PartId,
                        new StackTrace()));
                }
            ...
            using (var transactionScope = new TransactionScope(TransactionScopeOption.RequiresNew))
            {
                ...
                {
                    // 增加待办
                    PendingJob.Todo(instance, workItem);
                }
                transactionScope.Complete();
            }
