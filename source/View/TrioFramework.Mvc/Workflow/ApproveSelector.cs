...
namespace Bingosoft.TrioFramework.Mvc.Workflow {
	...
	public class ApproveSelector : Selector {
		...
		public ApproveSelector(WorkflowInstance instance) : this() {
		...
		public void SetDefault(string choice, string userid) {
			if (m_Users.ContainsKey(choice)) {
				foreach (var item in m_Users[choice]) {
		...
		public override string ToString() {
			...
			sbString.Append("\"needChoice\":\"" + this.m_NeedChoice + "\",");
			...
			foreach (var choice in m_NextChoice) {
				sbString.Append("\"" + choice + "\" : \"" + m_Activities[choice] + "\",");
			}
			sbString.Remove(sbString.Length - 1, 1);
			sbString.Append("},");
			foreach (var choice in m_NextChoice) {
