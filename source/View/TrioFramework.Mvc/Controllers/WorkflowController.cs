    ...
    public class WorkflowController : BaseController
    {
        ...
        public ActionResult Render(WorkflowForm form, string activityName)
        {
            ...
            if (this.handlers.ContainsKey(handlerKey))
            {
                ...
                var context = new WorkflowContext() { InstanceNo = form.InstanceNo, Form = xForm };
                var viewResult = this.handlers[handlerKey].Render(context, form);
                if (viewResult != null)
                {
        ...
        public ActionResult Save(WorkflowForm form)
        {
            try
            {
                ...
                var context = new WorkflowContext() {
                    InstanceNo = xForm.InstanceNo,
                    FormType = this.BusinessForm,
                    Form = xForm.BusinessForm
                };

                if (this.handlers.ContainsKey(xForm.HandlerKey))
                {
                    var func = this.handlers[xForm.HandlerKey];
                    if (func != null)
                    {
                        try
                        {
                            func.Save(context, xForm.ActionMode);
                        }
                ...
                if (form.ActionMode == ActionMode.Submit || form.ActionMode == ActionMode.Rollback)
                {
                    ...
                    if (this.handlers.ContainsKey(xForm.HandlerKey))
                    {
                        ...
                        if (func != null)
                        {
                            try
                            {
                                func.ResolveActor(context, selector);
                            }
                            catch (BusinessException ex)
                            {
                                return Error(500, ex.Message);
        ...
        public ActionResult Submit(WorkflowForm form)
        {
            try
            {
                ...
                // 自定义事件上下文参数
                var context = new WorkflowContext() { InstanceNo = form.InstanceNo, FormType = this.BusinessForm };

                // 处理流程提交前自定义事件
                if (this.handlers.ContainsKey(form.HandlerKey))
                {
                    var func = this.handlers[form.HandlerKey];
                    if (func != null)
                    {
                        try
                        {
                            func.BeforeSubmit(context, form.ApproveResult.Choice);
                        }
                        catch (BusinessException ex)
                        {
                            return Error(500, ex.Message);
                        ...
                        try
                        {
                            func.ResolveTobeReadActor(context, form.TobeReadSelector);
                        }
                        catch (BusinessException ex)
                        {
                            return Error(500, ex.Message);
                ...
                if (runSuccess)
                {
                    ...
                    if (this.handlers.ContainsKey(form.HandlerKey))
                    {
                        ...
                        if (func != null)
                        {
                            try
                            {
                                func.AfterSubmit(context, form.ApproveResult.Choice);
                            }
                            catch (BusinessException ex)
                            {
                                return Error(500, ex.Message);
